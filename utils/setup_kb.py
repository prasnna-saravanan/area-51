...
def create_jira_kb(api_key: str, azure_config: dict = None, use_pgvector: bool = True) -> None:
    ...
    embedding_model = {
        ...
        "model_name": azure_config.get("deployment", "text-embedding-3-large") if azure_config else "text-embedding-3-small",
        ...
        "api_version": azure_config.get("api_version", "2024-02-01") if azure_config else None,
    ...
    reranking_model = {
        "provider": "azure_openai" if azure_config else "openai",
        "model_name": azure_config.get("inference_deployment", "gpt-4.1") if azure_config else "gpt-4o",
        "api_key": azure_config.get("api_key") if azure_config else api_key,
        "base_url": azure_config.get("endpoint") if azure_config else None,
        "api_version": azure_config.get("api_version", "2024-02-01") if azure_config else None,
        "deployment": azure_config.get("inference_deployment") if azure_config else None,
        "rate_limit": 20 if azure_config else None,
    }
    ...
    kb_params = {
        ...
        "embedding_model": embedding_model,
        "reranking_model": reranking_model,
        "metadata_columns": ['id', 'key', 'project_id', 'project_key', 'project_name', 'priority', 'creator', 'assignee', 'status'],
        "content_columns": ['summary', 'description'],
        "id_column": 'id',
    }

    if use_pgvector:
        kb_params["storage"] = server.databases.jira.tables.jira_content
...
def create_zendesk_kb(api_key: str, azure_config: dict = None, use_pgvector: bool = True) -> None:
    ...
    kb_params = {
        ...
        "reranking_model": reranking_model,
        "metadata_columns": [
            "id", "status", "priority", "type", "assignee_id", "requester_id",
    ...
    if use_pgvector:
        kb_params["storage"] = server.databases.zendesk.tables.zendesk_tickets
